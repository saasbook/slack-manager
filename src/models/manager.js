...
class manager {
  ...
  bindEvents_() {
    ...
    this.controller.hears(["start meeting"], ["ambient", "direct_mention"], (bot, message) => {
    ...
    this.controller.hears(["status"], "direct_mention", (bot, message) => {
      bot.reply(
        message,
        "Active meetings are " + JSON.stringify(that.meetings)
      );
    });
...
> \`skip\`
* Skips the current user's turn. Asks to the skipped users again at the end of the meeting. Can be skipped more than once.
> \`dismiss\`
...
`
    ...
    this.controller.hears(["skip", "dismiss"], "ambient", (bot, message) => {
    ...
    this.controller.hears(["quit"], "ambient", (bot, message) => {
      let meeting = that.meetings[message.channel];
